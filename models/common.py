...
class ACmix(nn.Module):
    def __init__(self, in_planes=8, out_planes=8, kernel_att=7, head=4, kernel_conv=3, stride=1, dilation=1):
...
# ### att
        ...
        if self.stride > 1: # 在 h，w方向按步长取值
            q_att = Acstride(q_att, self.stride)
            q_pe = Acstride(pe, self.stride)
        else:
            q_pe = pe
...
class DecoupledHead(nn.Module):
    def __init__(self, ch=256, nc=3,  anchors=()):
        super().__init__()
        ...
        self.stride = 1
        self.dilation = 1
        self.acmix = ACmix(ch, self.out_planes, self.kernel_att, self.head, self.kernel_conv, self.stride, self.dilation)
...
# ----------- axmix decouple --------------
class ACmixDecoupledHead(nn.Module):
    def __init__(self, ch=256, nc=3,  anchors=()):
        super().__init__()
        self.nc = nc  # number of classes
        self.nl = len(anchors)  # number of detection layers
    ...
    def forward(self, x):
        ...
        return out
# ----------- atrous decouple --------------
class AtrousDecoupledHead(nn.Module):
    def __init__(self, ch=256, nc=3,  anchors=()):
        super().__init__()
        self.nc = nc  # number of classes
        self.nl = len(anchors)  # number of detection layers
        self.na = len(anchors[0]) // 2  # number of anchors
        self.merge = Conv(ch, 256 , 1, 1)
        self.cls_convs1 = AConv_r2(256 , 256,2)
        self.cls_convs2 = AConv_r3(256 , 256,2)
        self.reg_convs1 = AConv_r2(256 , 256,2)
        self.reg_convs2 = AConv_r3(256 , 256,2)
        self.cls_preds = nn.Conv2d(256 , self.nc * self.na, 1)
        self.reg_preds = nn.Conv2d(256 , 4 * self.na, 1)
        self.obj_preds = nn.Conv2d(256 , 1 * self.na, 1)
        # self.in_planes = 256
...
class ASPPFCSPC(nn.Module):
    
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=5):
        super(ASPPFCSPC, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
        self.cv1 = AConv_r2(c1, c_, 2, 1)
        self.cv2 = AConv_r2(c1, c_, 2, 1)
        self.cv3 = Conv(c_, c_, 3, 1)
        self.cv4 = AConv_r2(c_, c_, 2, 1)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
        self.cv5 = AConv_r2(4 * c_, c_, 2, 1)
        self.cv6 = Conv(c_, c_, 3, 1)
        self.cv7 = AConv_r2(2 * c_, c2, 2, 1)
