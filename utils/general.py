...
def clip_coords(boxes, img_shape):
    ...
    # mpdiou
def bbox_iou(box1, box2, xywh=True, MDIoU=False, GIoU=False, DIoU=False, CIoU=False, MDPIoU=False, hw=None, eps=1e-7):
    ...
    # 中心点距离
    b1_centerx = b1_x1 + ((b1_x2-b1_x1) / 2)
    b1_centery = b1_y1 + ((b1_y2-b1_y1) / 2)

    b2_centerx = b2_x1 + ((b2_x2-b2_x1) / 2)
    b2_centery = b2_y1 + ((b2_y2-b2_y1) / 2)
    # print("b1 x1 -----------",b1_x1.size())
    # print("b1 venterx -----------",b1_centerx.size())
    d = torch.sqrt((b1_centerx-b2_centerx) ** 2 + (b1_centery-b2_centery) ** 2)
    # 最大外包括对角线距离
    bound_l = torch.min(b1_x1,b2_x1)
    bound_r = torch.max(b1_x2,b2_x2)
    bound_t = torch.min(b1_y1,b2_y1)
    bound_b = torch.max(b1_y2,b2_y2)
    c = torch.sqrt((bound_r-bound_l) ** 2 + (bound_t - bound_b) ** 2)
    ...
    if CIoU or DIoU or GIoU:
        ...
        d2 = (b2_x2 - b1_x2) ** 2 + (b2_y2 - b1_y2) ** 2
        # print("d1 --- " , d1)
        mpdiou = iou - d1 / hw - d2 / hw
        mdiou = mpdiou - (d ** 2) / (c ** 2)
        return mdiou  # MPDIoU
    return iou  # IoU
