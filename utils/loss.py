    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # print("hw----------",tobj.size()[2] * tobj.size()[3])
                # MDIoU
                iou = bbox_iou(pbox, tbox[i], MDIoU=True, hw=tobj.size()[2] * tobj.size()[3]).squeeze()
                lbox += (1.0 - iou).mean()  # iou loss
                # Ciou
                ...
                #     iou = iou.squeeze()
                # -------------- src ---------------------------
                # iou = bbox_iou(pbox.T, tbox[i],  CIoU=True, x1y1x2y2=False)  # iou(prediction, target)
                # lbox += (1.0 - iou).mean()  # iou loss
